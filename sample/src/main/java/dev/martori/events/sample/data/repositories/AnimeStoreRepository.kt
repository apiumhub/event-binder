...
class AnimeStoreRepository(api: AnimeApi) : AnimeRepository {
    private val store = StoreBuilder.fromNonFlow { request: AnimeRequest -> api.getAnimeList(request.count).map { it.toDomain() } }.build()

    override suspend fun get(key: AnimeRequest): List<Anime> = store.get(key)
}